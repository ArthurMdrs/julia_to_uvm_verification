...
gen_refmod_base() = begin
    ...
    my_str *= """
        ...
        seq_item_t m_seq_item;

        uvm_analysis_port#(seq_item_t) m_$(rm_name)_port;

        function new(string name="$(dut_name)_$(rm_name)", uvm_component parent = null);
            super.new(name, parent);
            m_$(rm_name)_port = new("m_$(rm_name)_port", this);
        endfunction : new
        ...
        function void write(seq_item_t t);
            m_seq_item = seq_item_t::type_id::create("m_seq_item");

            m_seq_item.copy(t);

            m_$(rm_name)_port.write(m_seq_item);

            `uvm_info("$(uppercase(dut_name)) REFMOD", \$sformatf("Processed item: \\n%s", m_seq_item.convert2string()), $(verbosities["refmod_proc_item"]))
        endfunction : write
