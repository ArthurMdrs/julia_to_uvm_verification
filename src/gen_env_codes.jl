...
gen_line_connect_sequencers(uvc_name, tabs, env_cfg_name) = begin
    ...
    $(tabs)    if (m_$(env_cfg_name).m_$(uvc_name)_$(cfg_name).m_is_active == UVM_ACTIVE) begin
...
get_sb_ports_conn(tabs, env_cfg_name) = begin
    ...
    my_str = """
    $(tabs)if (m_$(env_cfg_name).m_has_scoreboard) begin
    $(tabs)    if (m_$(env_cfg_name).m_has_$(uvc_names[1])_agent)
    $(tabs)        m_$(uvc_names[1])_$(agent_name).m_item_from_monitor_port.connect(m_$(dut_name)_$(sb_name).m_item_from_monitor_fifo.analysis_export);
    $(tabs)end
    """
    return my_str
end
get_rm_ports_conn(tabs, env_cfg_name) = begin
    ...
    agent_name = get_uvc_cfg_fld(uvc_names[1], :class_names)["agent"]
    my_str = """
    $(tabs)if (m_$(env_cfg_name).m_has_refmod) begin
    $(tabs)    if (m_$(env_cfg_name).m_has_$(uvc_names[1])_agent)
    $(tabs)        m_$(uvc_names[1])_$(agent_name).m_item_from_monitor_port.connect(m_$(dut_name)_$(rm_name).analysis_export);
    $(tabs)end
    """
    if gen_scoreboard
        my_str *= "$(tabs)if (m_$(env_cfg_name).m_has_refmod && m_$(env_cfg_name).m_has_scoreboard)\n"
        my_str *= "$(tabs)    m_$(dut_name)_$(rm_name).m_$(rm_name)_port.connect(m_$(dut_name)_$(sb_name).m_item_from_refmod_fifo.analysis_export);\n"
        my_str = """
        $(tabs)if (m_$(env_cfg_name).m_has_refmod && m_$(env_cfg_name).m_has_scoreboard) begin
        $(tabs)    m_$(dut_name)_$(rm_name).m_$(rm_name)_port.connect(m_$(dut_name)_$(sb_name).m_item_from_refmod_fifo.analysis_export);
        $(tabs)end
        """
    end
    return my_str
...
get_cov_ports_conn(tabs, env_cfg_name) = begin
    ...
    agent_name = get_uvc_cfg_fld(uvc_names[1], :class_names)["agent"]
    my_str = """
    $(tabs)if (m_$(env_cfg_name).m_has_coverage) begin
    $(tabs)    if (m_$(env_cfg_name).m_has_$(uvc_names[1])_agent)
    $(tabs)        m_$(uvc_names[1])_$(agent_name).m_item_from_monitor_port.connect(m_$(dut_name)_$(cov_name).analysis_export);
    $(tabs)end
    """
    return my_str
end
...
gen_line_has_agent(uvc_name, tabs) = begin
    my_str = """
    $(tabs)bit m_has_$(uvc_name)_agent;
    """
    return my_str
end
gen_line_has_agent_assign(uvc_name, tabs) = begin
    my_str = """
    $(tabs)m_has_$(uvc_name)_agent = 1;
    """
    return my_str
end
...
gen_env_base() = begin
    ...
    my_str *= """
            ...
            if (m_$(env_cfg_name).m_has_virtual_sequencer) begin
    ...
    my_str *= gen_refmod ? """
            ...
            if (m_$(env_cfg_name).m_has_refmod) begin
    ...
    my_str *= gen_scoreboard ? """
            // Create scoreboard
            if (m_$(env_cfg_name).m_has_scoreboard) begin
    ...
    my_str *= env_has_coverage ? """
            ...
            if (m_$(env_cfg_name).m_has_coverage) begin
...
gen_env_cfg() = begin
    ...
    $( gen_long_str(uvc_names, "    ", gen_line_has_agent)[1:end-1] )
        ...
        bit m_has_virtual_sequencer;
    """

    if env_has_coverage
        my_str *= "    bit m_has_coverage;\n"
    end
    if gen_scoreboard
        my_str *= "    bit m_has_scoreboard;\n"
    end
    if gen_refmod
        my_str *= "    bit m_has_refmod;\n"
    end
    ...
    my_str *="""
        ...
        function new (string name = "$(dut_name)_$(env_cfg_name)");
            ...
            m_has_virtual_sequencer = 1'b1;
    """

    if env_has_coverage
        my_str *= "        m_has_coverage = 1'b1;\n"
    end
    if gen_scoreboard
        my_str *= "        m_has_scoreboard = 1'b1;\n"
    end
    if gen_refmod
        my_str *= "        m_has_refmod = 1'b1;\n"
    end
