...
gen_if_base(prefix_name) = begin
    ...
    $( gen_lines_mon_cb(if_sigs_vec, "    ", clock_name)[1:end-1] )
        ...
        task static $(prefix_name)_reset ();
    """
    ...
    $( gen_long_str(if_sigs_vec, "        ", gen_line_reset_sig)[1:end-1] )
        ...
        task static send_to_dut ($(prefix_name)_$(tr_name)_t req);
            @(drv_cb);
        ...
        task static collect_tr ($(prefix_name)_$(tr_name)_t req);
            @(mon_cb);
...
gen_clknrst_if(prefix_name) = begin
    ...
    interface $(prefix_name)_$(if_name) $(param_str)(
        ...
        task static assert_rst (int unsigned rst_assert_duration);
        ...
        task static wait_clk_posedge ();
            @(posedge $(clock_name));
        endtask : wait_clk_posedge

        task static wait_clk_negedge ();
            @(negedge $(clock_name));
