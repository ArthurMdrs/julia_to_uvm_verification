...
gen_agent_base(prefix_name) = begin
    ...
    my_str *= """
        ...
        uvm_analysis_port #($(prefix_name)_$(tr_name)_t) m_item_from_monitor_port;

        function new (string name, uvm_component parent);
            super.new(name, parent);
            m_item_from_monitor_port = new("m_item_from_monitor_port", this);
        endfunction : new
    ...
    """
    my_str *= agent_has_coverage ? """
            if (m_$(cfg_name).m_has_coverage == 1'b1) begin
    ...
    my_str *= """
        ...
        function void connect_phase (uvm_phase phase);
            ...
            if (m_$(cfg_name).m_has_monitor == 1'b1) begin
                m_monitor.m_item_collected_port.connect(m_item_from_monitor_port);
            end
    ...
    """
    my_str *= agent_has_coverage ? """
            if (m_$(cfg_name).m_has_coverage == 1'b1 && m_$(cfg_name).m_has_monitor == 1'b1) begin
                m_monitor.m_item_collected_port.connect(m_$(prefix_name)_$(cov_name).analysis_export);
            end
    """ : ""
    my_str *= """
