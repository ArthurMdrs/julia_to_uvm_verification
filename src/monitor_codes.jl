...
get_normal_mon_funcs(prefix_name) = begin
    ...
    my_str *= """
        task collect ();
            forever begin
                m_mon_$(tr_name) = $(tr_type)::type_id::create("m_mon_$(tr_name)", this);

                void'(begin_tr(m_mon_$(tr_name), "$(uppercase(prefix_name))_MONITOR_$(uppercase(tr_name))"));
                vif.collect_tr(m_mon_$(tr_name));
                end_tr(m_mon_$(tr_name));

                `uvm_info("$(uppercase(prefix_name)) MONITOR", \$sformatf("Transaction Collected:%s", m_mon_$(tr_name).convert2string()), $(verb_dict["collect_tr"]))
                m_item_collected_port.write(m_mon_$(tr_name));
                m_num_tr_col++;
            end
        endtask : collect
    ...
    my_str =  """
        task run_phase (uvm_phase phase);
            ...
            forever begin
                ...
                // end_tr(tr);
                // m_item_collected_port.write(tr);
                // m_num_tr_col++;
            end
        endtask : run_phase
...
gen_monitor(prefix_name, type::uvc_class_type) = begin
    ...
    $( gen_line_vif_typedef(prefix_name, "    ")[1:end-1] )
        ...
        $(tr_type) m_mon_$(tr_name);
        int m_num_tr_col;

        uvm_analysis_port #($(tr_type)) m_item_collected_port;
    ...
    my_str *= """
        ...
        function void report_phase(uvm_phase phase);
            `uvm_info("$(uppercase(prefix_name)) MONITOR", \$sformatf("Report: $(uppercase(prefix_name)) MONITOR collected %0d transactions", m_num_tr_col), $(verb_dict["monitor_report"]))
